package day22;

public class Day22Task1 {
    public static void main(String[] args) {
        String[] boardAndMovement = INPUT.split("\n\n");

        String[] board = boardAndMovement[0].split("\n");

        int dir = 0;
        int row = 0;
        int col = board[0].indexOf('.');

        int move = 0;
        String s = boardAndMovement[1] + ' ';
        for (char c : s.toCharArray()) {
            if (c == 'R' || c == 'L' || c == ' ') {
                System.out.println(move + " " + dir + " " + c);
                if (move > 0) {
                    int lastmove = move;
                    move = 0;
                    moveloop: for (int i = 0; i < lastmove; i++) {
                        int nextrow = dir == 1 ? row + 1 : dir == 3 ? row - 1 : row;
                        int nextcol = dir == 0 ? col + 1 : dir == 2 ? col - 1 : col;
                        if (dir == 0) {
                            if (nextcol >= board[row].length() || board[row].charAt(nextcol) == ' ') {
                                int firstFree = board[row].indexOf('.');
                                int firstBlocked = board[row].indexOf('#');
                                if (firstBlocked >= 0 && firstBlocked < firstFree) {
                                    // blocked
                                    break;
                                }
                                nextcol = firstFree;
                            } else if (board[row].charAt(nextcol) == '#') {
                                // blocked
                                break;
                            }
                        } else if (dir == 2) {
                            if (nextcol < 0 || board[row].charAt(nextcol) == ' ') {
                                int firstFree = board[row].lastIndexOf('.');
                                int firstBlocked = board[row].lastIndexOf('#');
                                if (firstBlocked >= 0 && firstBlocked > firstFree) {
                                    // blocked
                                    break;
                                }
                                nextcol = firstFree;
                            } else if (board[row].charAt(nextcol) == '#') {
                                // blocked
                                break;
                            }
                        } else if (dir == 1) {
                            if (nextrow >= board.length || board[nextrow].length() <= col || board[nextrow].charAt(col) == ' ') {
                                for (int j = 0; j < board.length; j++) {
                                    char cto = board[j].length() <= col ? ' ' : board[j].charAt(col);
                                    if (cto == '#') {
                                        break moveloop;
                                    } else if (cto == '.') {
                                        nextrow = j;
                                        break;
                                    }
                                }
                            } else if (board[nextrow].charAt(col) == '#') {
                                // blocked
                                break;
                            }
                        } else if (dir == 3) {
                            if (nextrow < 0 || board[nextrow].length() <= col || board[nextrow].charAt(col) == ' ') {
                                for (int j = board.length - 1; j >= 0; j--) {
                                    char cto = board[j].length() <= col ? ' ' : board[j].charAt(col);
                                    if (cto == '#') {
                                        break moveloop;
                                    } else if (cto == '.') {
                                        nextrow = j;
                                        break;
                                    }
                                }
                            } else if (board[nextrow].charAt(col) == '#') {
                                // blocked
                                break;
                            }
                        } else {
                            throw new IllegalStateException("dir=" + dir);
                        }
                        row = nextrow;
                        col = nextcol;
                    }
                    System.out.println(row + " " + col + " " + dir + " -> " + ((row + 1) * 1000 + (col + 1) * 4 + dir));
                    dir = Math.floorMod(c == 'R' ? dir + 1 : c == 'L' ? dir - 1 : dir, 4);
                }
            } else {
                move = 10 * move + (c - '0');
            }
        }
        System.out.println(row + " " + col + " " + dir + " -> " + ((row + 1) * 1000 + (col + 1) * 4 + dir));
    }

    public static final String INPUT_TEST = "        ...#\n"
            + "        .#..\n"
            + "        #...\n"
            + "        ....\n"
            + "...#.......#\n"
            + "........#...\n"
            + "..#....#....\n"
            + "..........#.\n"
            + "        ...#....\n"
            + "        .....#..\n"
            + "        .#......\n"
            + "        ......#.\n"
            + "\n"
            + "10R5L5R10L4R5L5";

    public static final String INPUT = "                                                  ....#..#..........#........#.#...........#.##.................#................#..#...............#.\n"
            + "                                                  .##......##......##......#.........#...............#.#.............#.....#.......................#.#\n"
            + "                                                  ....#.............#....##..............#............#..........#.#..................................\n"
            + "                                                  ..........#..#...........#......#....#......................#........#.............#......#..#......\n"
            + "                                                  .......#.......#.#...........#.....................#....#....#.......#....................#...#.....\n"
            + "                                                  ......#..........#.....................................#.#....#.#...##...#...............#........#.\n"
            + "                                                  #............#..............##..#..#..#.......#........##.....#.#.#.........#..#....................\n"
            + "                                                  ...............#...........#.....................................#..............#.....#.#...........\n"
            + "                                                  ................#..........#............#..........#...#.......#...................#................\n"
            + "                                                  ....#.....#..................#..##.............#.........#.............#......#.#....#...#..#.....#.\n"
            + "                                                  ..........#.......#..#.................#..................................#..#.#...#.......#........\n"
            + "                                                  ..........#.......................#.#..#...#...#.......#............#.##...#..........#..#.#........\n"
            + "                                                  ....#........#...........#............#..#........#..#.......#.....#.........##.#............#......\n"
            + "                                                  .#...................#....#.........#......##............#.............#.......#..............#.....\n"
            + "                                                  .........................................................#.......#....#.........#...................\n"
            + "                                                  .#.............#...#.....#...........................#.........#..........#.#.......#......#......#.\n"
            + "                                                  .#.........#..#...#....#.##.................#....#.......#................##.......#....#...........\n"
            + "                                                  ....................#.............................#.........................#.......................\n"
            + "                                                  .......#..............#...##...........................#...............#............................\n"
            + "                                                  ......................#..#.....#...#........................#..#......#..............#..#....#......\n"
            + "                                                  ...............#..........#.................#........##.....#...#.........#.................#.......\n"
            + "                                                  ....#..................................#.....#......##.......#.....................#...#.........#..\n"
            + "                                                  .#..#..............................###.......#..............#....#.......................#....#.....\n"
            + "                                                  .#..........................#..#......#..............#.........#............................#.#.###.\n"
            + "                                                  .................#......#......................................#..........#.....#......##.......#...\n"
            + "                                                  .....##..............#......#.........................#..#...#..#.............#.................#...\n"
            + "                                                  #..#......#..##.....#........#.............#......#...#...............................#....#.......#\n"
            + "                                                  .#....##...........#...#.......#.......#..............#......#........#....#.......#..........#.....\n"
            + "                                                  .....#.........#............#.....##..#.............#.....#...#........#............................\n"
            + "                                                  ..###.......#......#......#.....#....#..#..............................................#............\n"
            + "                                                  ..........#.....................#........................#..#....#............#...#....#............\n"
            + "                                                  ......#..##...............##...............#.............#........#..............#.....#............\n"
            + "                                                  ......#.....................#...........#.......#.#.....................#..............#....#.......\n"
            + "                                                  ........#......#.#.........#...#...........#..#...........#.........................................\n"
            + "                                                  .....#.....#..........#......................#.............#.......#.......#.............#..#.......\n"
            + "                                                  ...#.........#.........#....#...............#.........#...........................#...#.##..#....#.#\n"
            + "                                                  .....#.........##..#...#.###................................#.......##......#.......................\n"
            + "                                                  #........#.........#..#...#...........##...................#.......#....#...........................\n"
            + "                                                  ...#.#......#.............#.#........#.............#...........#..........#......#..#...#......#....\n"
            + "                                                  ...##........#..#.#...............#.....#............##......#.......#..#..#....#...................\n"
            + "                                                  ..........#............#..#.......#..........#.#.................................#..................\n"
            + "                                                  ....#....#............................#..........#.#...............#................................\n"
            + "                                                  ............#.#.#................#.......#...#...........................#..........................\n"
            + "                                                  ....#........#....#..........#......#....#................#..............#........#....#...........#\n"
            + "                                                  .....#.......#........................#.............#...#..##........#..................#...........\n"
            + "                                                  ......##....#..................................................#.#.........#....#......#............\n"
            + "                                                  ..........#.#..................................#.................................#.....#....#.......\n"
            + "                                                  ................#....#.#.#...........#.......#..###..#..................#...#....#....#...#....#....\n"
            + "                                                  .........#....#.#...#.......#..........................................#.........#.#.........#...#..\n"
            + "                                                  .........................#.....#.......................#........#............#.......#...#..........\n"
            + "                                                  .......#.......#.....#.#........................#.\n"
            + "                                                  .................#......#.........#.#.............\n"
            + "                                                  ....##.............#....#.......#..#..........#...\n"
            + "                                                  .............###...................#.#..........#.\n"
            + "                                                  .#.............#...........#......#.....#........#\n"
            + "                                                  .......#.......................#..............#...\n"
            + "                                                  .......#...#......................................\n"
            + "                                                  .......#.#.....#......#...........#..............#\n"
            + "                                                  ...............#......#.#................#........\n"
            + "                                                  .................#.....#.......#.........#..#..#..\n"
            + "                                                  .........#...........................#............\n"
            + "                                                  ..#...#........#.........#.............#.......#..\n"
            + "                                                  .........................#....................#...\n"
            + "                                                  ......#.#..........#........#.....#...............\n"
            + "                                                  ..#.....#..#.....#.................#....#..#......\n"
            + "                                                  ...#.........#........................#.......##..\n"
            + "                                                  ..#.....#..#......................................\n"
            + "                                                  ........#................#.............##.........\n"
            + "                                                  ......#..........#......#.....#..........#........\n"
            + "                                                  ...........#........#...#.........................\n"
            + "                                                  ...................#..........#......#............\n"
            + "                                                  ......#......#.#.#.........................#...#..\n"
            + "                                                  ............##...............#..#.#.#........#....\n"
            + "                                                  ....#...#..#..#...#.....#.........................\n"
            + "                                                  ..........#........#............................#.\n"
            + "                                                  .......#................#.........................\n"
            + "                                                  ...........#.................##.............#.....\n"
            + "                                                  ......#..#.........#......................#.......\n"
            + "                                                  .......#.....#........#...#..#......#...#.......#.\n"
            + "                                                  .........#..#..........#....#...........#.....#...\n"
            + "                                                  ...#..#..............#................#...........\n"
            + "                                                  ......#....................#..#...#.......#.......\n"
            + "                                                  .#..#........#.......#..................#.........\n"
            + "                                                  ............#.....#.#..#.#.#...#.........#...#....\n"
            + "                                                  ...............#.#.#...........#.#............#...\n"
            + "                                                  ..........##......#.........#..............#......\n"
            + "                                                  ....##......#..................#.#................\n"
            + "                                                  ..............#...#................#..............\n"
            + "                                                  .#......#........#.........#.........##...........\n"
            + "                                                  .....................#..................#.#.......\n"
            + "                                                  ........#..#..........#.....................#.....\n"
            + "                                                  ............#..#....#...................#.........\n"
            + "                                                  .......#......#.........#..........#..#.........#.\n"
            + "                                                  ...#.#..........#...............#...##..#.......#.\n"
            + "                                                  ..#.#.#.#...#.##......#.......#..#.#..#...........\n"
            + "                                                  .......#.#.....#......#..#.......#................\n"
            + "                                                  ..#.............................#.#...#....#......\n"
            + "                                                  ..#................#.....#....#.................#.\n"
            + "                                                  ................#.....#.....#......#.............#\n"
            + "                                                  ..............................#............#......\n"
            + "..#...........#.##.#..##..................#....#...##.....................#.....#...................\n"
            + ".....#......#.#...#.....#................#....................#...............................#.....\n"
            + "...#.......................#.......#..#............#.#....##..#...............#..#.#....#.........#.\n"
            + "#...............#.....#..#......##.#.....#......................#.....#...#.........................\n"
            + "...........#.........#....................................................#.............#...........\n"
            + "............##..........#..#...................#..............#...................#....##...........\n"
            + "....#.............##......#..............####.........#....#.......#.................#......#.......\n"
            + "..........#.......#...#.#............................#..................#...........................\n"
            + "........................#..............#............#......#....#...........#......#..........#.....\n"
            + ".#.............................................#..#.#.....................#...............#.........\n"
            + "#.................................................#................#...........#.......#...#........\n"
            + "#.............#.#............##..#...#...........#.............#.................................#..\n"
            + "..................#.......#.....#....#.................#..........#....#.#..#..........#...#........\n"
            + "..........................##..................#...#....................#............................\n"
            + "................#.......#......#.....#.##...........#..#..........#.......#........#..#.............\n"
            + "........#....##....................#................................#.#.....#..#....................\n"
            + ".#..............#..#................#.....#............#....#...............#.......................\n"
            + ".........#.....#................................#.......#....................#..#...................\n"
            + "................#.....#.........#............#.............#..................#......#.....#.##.....\n"
            + "#.........#......................................#..........#........#..#...#...........#...#.......\n"
            + "#.......#..#...#.#.#.#............#..................#...............#..............#.......#.......\n"
            + "........#.....#.....#.....#..#.#...................#....#.......................#...#.....#..#...#..\n"
            + ".........#............................#...........#...#...#.#..........##....#...................#..\n"
            + "......#..................#.................#............#......#..#.................#............##.\n"
            + ".........#...............#..........#..............#..#....#........................#...........#...\n"
            + "..........#....#..#......#..#.............#...#............##............................#.......#..\n"
            + "....#.............#.#..#...........#.#..#..##.#....................................##...............\n"
            + ".....................#.#...##..........#....................#...#................#...#.........#....\n"
            + "...#.#..........................#....#.................#....#.....................#.#...............\n"
            + ".#..........#.........#...............#........##.#................#.......#..............##.#......\n"
            + ".....#........#...#..............#........#...............#....#......................#.............\n"
            + "..........#....#.........................#............#........#...................#............#...\n"
            + "............................................#.............................#...#.....................\n"
            + ".........#........#............#...................#.............#............#...........#.........\n"
            + "..............#.#.#..#............##..................#............#................................\n"
            + "....................#..#.......#............#.................#......#......#........#.#...........#\n"
            + "#....#...##...#.........................##..............#..........#..............#.................\n"
            + ".....#...............#..........#..........#.........#............#..............#..................\n"
            + "............................#...#...........#..#.#.....#.#......#..............#..#.........#.......\n"
            + "......#...........#................#.....................#...#..............#................#....#.\n"
            + ".##..#....#...##........#..................#.#.........#...#.......#.#..........#........#......#...\n"
            + "#.........#.....................##.#...........#....#.#.............#....#........#..#..............\n"
            + ".............................#....#...........#.#....#.#..............#.............#............#.#\n"
            + "........#....#.........#.............#.##.......#...............#.#....#..#.................#......#\n"
            + "...........#..#.......#.#.....#.#.................#..........#..........................#..#..#...#.\n"
            + "............#.....#......#.#.....#...#..##...............#...#......#........................##.....\n"
            + "....##...#.#..........#.........#....#....#.......#...............#....#.........................#..\n"
            + "............#.....#............#....#.....##.......#.....#.........................#...#............\n"
            + ".......#.....#.....................##...........##.......................#.#....#.........#.........\n"
            + "................#.........................................#.........#.#.....#....#.............#....\n"
            + "#.........................#..........#.......#....\n"
            + ".......................#....#.............#.......\n"
            + "...................#........................#.....\n"
            + "....#.#.........#..##..........................#..\n"
            + "..........#.#......#.........#....#...............\n"
            + "..##...........#........#................#........\n"
            + ".......#....#..#...#.##...#.....................#.\n"
            + "......##...#.....#.................#............#.\n"
            + "......#...#...........#....#.......#..............\n"
            + "...#.........#.....#.......#.....##...#.....#.....\n"
            + "..........#....#............#..#..................\n"
            + "..#.#.......................#........#............\n"
            + ".......##.#.##.............#.....#...........#....\n"
            + "..............#....#........#.....#...............\n"
            + "...........................#......#..............#\n"
            + "....#........#.....#......#............#.#.#......\n"
            + ".......#.....#...#................#.....#.........\n"
            + "..............#.........#..........#......#....#..\n"
            + ".#.#..#.....#.........................#......#.#..\n"
            + "......#...#.......#..#..........................##\n"
            + "......#......................................#.#.#\n"
            + ".......#.....#....#.....#..##......#...#..........\n"
            + ".........#..#...#.........#..............#....#...\n"
            + "..#............#.#.............#........#........#\n"
            + ".............#..........#.##......................\n"
            + "#.....#......#....#..................#............\n"
            + "...#........#....................#................\n"
            + "#....#...........##.#......#....#...........##..#.\n"
            + "##...........#..........#....................#....\n"
            + ".#...............#..............#.........#.....#.\n"
            + ".........#...#..................................#.\n"
            + "....................#.#.....#................#....\n"
            + "....#......................................#......\n"
            + "#.................##...#..#...#............#.#.#..\n"
            + "....#........#.#...........#...............#......\n"
            + ".......#...........#....................#.........\n"
            + "...#............##.....#.#............#....#......\n"
            + "#..#...##..#..............#.#.....................\n"
            + "..........#.#......#......##.......#.....#........\n"
            + ".##..................#............................\n"
            + ".....#....................#.......#...............\n"
            + ".................#.#..#...#............#...#.#....\n"
            + "........................#..#.................#....\n"
            + ".#.....#...............##........#.......#........\n"
            + "........#......#.............................#..#.\n"
            + ".........................#...#.......#.......#....\n"
            + "#.......#..................#..#................#..\n"
            + "......#..........#.......##.....#........#........\n"
            + "#.#....#................##......#......#..........\n"
            + "..#.#..............#.........#...................#\n"
            + "\n"
            + "";
}
