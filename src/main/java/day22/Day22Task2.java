package day22;

public class Day22Task2 {
    public final static int DIR_RIGHT = 0;
    public final static int DIR_DOWN = 1;
    public final static int DIR_LEFT = 2;
    public final static int DIR_UP = 3;

    public static void main(String[] args) {
        String[] boardAndMovement = INPUT.split("\n\n");
        String[] board = boardAndMovement[0].split("\n");
        String movementString = boardAndMovement[1] + ' ';
        int segmentSize = 50;

        // initial pose
        int dir = DIR_RIGHT;
        int row = 0;
        int col = board[0].indexOf('.');

        int move = 0;
        for (char c : movementString.toCharArray()) {
            if (c == 'R' || c == 'L' || c == ' ') {
                System.out.println("move " + move + " " + dir + " rotate " + c);
                if (move > 0) {
                    int lastmove = move;
                    move = 0;
                    for (int i = 0; i < lastmove; i++) {
                        System.out.println("  from " + row + " " + col + " " + dir);
                        int nextdir = dir;
                        int nextrow = nextdir == 1 ? row + 1 : nextdir == 3 ? row - 1 : row;
                        int nextcol = nextdir == 0 ? col + 1 : nextdir == 2 ? col - 1 : col;

                        // if we would go outside the field adjust next position and rotation
                        if (dir == DIR_RIGHT) {
                            if (nextcol >= board[row].length() || board[row].charAt(nextcol) == ' ') {
                                if (row < segmentSize) {
                                    nextdir = DIR_LEFT;
                                    nextrow = segmentSize * 3 - 1 - row;
                                    nextcol = segmentSize * 2 - 1;
                                } else if (row < segmentSize * 2) {
                                    nextdir = DIR_UP;
                                    nextrow = segmentSize - 1;
                                    nextcol = segmentSize * 1 + row;
                                } else if (row < segmentSize * 3) {
                                    nextdir = DIR_LEFT;
                                    nextrow = segmentSize * 3 - 1 - row;
                                    nextcol = segmentSize * 3 - 1;
                                } else {
                                    nextdir = DIR_UP;
                                    nextrow = segmentSize * 3 - 1;
                                    nextcol = row - segmentSize * 2;
                                }
                            }
                        } else if (dir == DIR_LEFT) {
                            if (nextcol < 0 || board[row].charAt(nextcol) == ' ') {
                                if (row < segmentSize) {
                                    nextdir = DIR_RIGHT;
                                    nextrow = segmentSize * 3 - 1 - row;
                                    nextcol = 0;
                                } else if (row < segmentSize * 2) {
                                    nextdir = DIR_DOWN;
                                    nextrow = segmentSize * 2;
                                    nextcol = row - segmentSize;
                                } else if (row < segmentSize * 3) {
                                    nextdir = DIR_RIGHT;
                                    nextrow = segmentSize * 3 - 1 - row;
                                    nextcol = segmentSize;
                                } else {
                                    nextdir = DIR_DOWN;
                                    nextrow = 0;
                                    nextcol = row - segmentSize * 2;
                                }
                            }
                        } else if (dir == DIR_DOWN) {
                            if (nextrow >= board.length || board[nextrow].length() <= col || board[nextrow].charAt(col) == ' ') {
                                if (col < segmentSize) {
                                    nextdir = DIR_DOWN;
                                    nextrow = 0;
                                    nextcol = col + segmentSize * 2;
                                } else if (col < segmentSize * 2) {
                                    nextdir = DIR_LEFT;
                                    nextrow = segmentSize * 2 + col;
                                    nextcol = segmentSize - 1;
                                } else {
                                    nextdir = DIR_LEFT;
                                    nextrow = col - segmentSize;
                                    nextcol = segmentSize * 2 - 1;
                                }
                            }
                        } else if (dir == DIR_UP) {
                            if (nextrow < 0 || board[nextrow].length() <= col || board[nextrow].charAt(col) == ' ') {
                                if (col < segmentSize) {
                                    nextdir = DIR_RIGHT;
                                    nextrow = segmentSize + col;
                                    nextcol = segmentSize;
                                } else if (col < segmentSize * 2) {
                                    nextdir = DIR_RIGHT;
                                    nextrow = segmentSize * 2 + col;
                                    nextcol = 0;
                                } else {
                                    nextdir = DIR_UP;
                                    nextrow = segmentSize * 4 - 1;
                                    nextcol = col - segmentSize * 2;
                                }
                            }
                        }
                        System.out.println("    to " + nextrow + " " + nextcol + " " + nextdir);
                        char nextField = board[nextrow].charAt(nextcol);
                        if (nextField == '#') {
                            System.out.println("    -> blocked!");
                            break;
                        } else if (nextField != '.') {
                            throw new IllegalStateException("walked of the dice!");
                        }
                        dir = nextdir;
                        row = nextrow;
                        col = nextcol;
                    }
                    dir = Math.floorMod(c == 'R' ? dir + 1 : c == 'L' ? dir - 1 : dir, 4);
                }
            } else {
                move = 10 * move + (c - '0');
            }
        }
        System.out.println("result: " + row + " " + col + " " + dir + " -> " + ((row + 1) * 1000 + (col + 1) * 4 + dir));
    }

    public static final String INPUT_TEST = "        ...#\n"
            + "        .#..\n"
            + "        #...\n"
            + "        ....\n"
            + "...#.......#\n"
            + "........#...\n"
            + "..#....#....\n"
            + "..........#.\n"
            + "        ...#....\n"
            + "        .....#..\n"
            + "        .#......\n"
            + "        ......#.\n"
            + "\n"
            + "10R5L5R10L4R5L5";

    public static final String INPUT = "                                                  ....#..#..........#........#.#...........#.##.................#................#..#...............#.\n"
            + "                                                  .##......##......##......#.........#...............#.#.............#.....#.......................#.#\n"
            + "                                                  ....#.............#....##..............#............#..........#.#..................................\n"
            + "                                                  ..........#..#...........#......#....#......................#........#.............#......#..#......\n"
            + "                                                  .......#.......#.#...........#.....................#....#....#.......#....................#...#.....\n"
            + "                                                  ......#..........#.....................................#.#....#.#...##...#...............#........#.\n"
            + "                                                  #............#..............##..#..#..#.......#........##.....#.#.#.........#..#....................\n"
            + "                                                  ...............#...........#.....................................#..............#.....#.#...........\n"
            + "                                                  ................#..........#............#..........#...#.......#...................#................\n"
            + "                                                  ....#.....#..................#..##.............#.........#.............#......#.#....#...#..#.....#.\n"
            + "                                                  ..........#.......#..#.................#..................................#..#.#...#.......#........\n"
            + "                                                  ..........#.......................#.#..#...#...#.......#............#.##...#..........#..#.#........\n"
            + "                                                  ....#........#...........#............#..#........#..#.......#.....#.........##.#............#......\n"
            + "                                                  .#...................#....#.........#......##............#.............#.......#..............#.....\n"
            + "                                                  .........................................................#.......#....#.........#...................\n"
            + "                                                  .#.............#...#.....#...........................#.........#..........#.#.......#......#......#.\n"
            + "                                                  .#.........#..#...#....#.##.................#....#.......#................##.......#....#...........\n"
            + "                                                  ....................#.............................#.........................#.......................\n"
            + "                                                  .......#..............#...##...........................#...............#............................\n"
            + "                                                  ......................#..#.....#...#........................#..#......#..............#..#....#......\n"
            + "                                                  ...............#..........#.................#........##.....#...#.........#.................#.......\n"
            + "                                                  ....#..................................#.....#......##.......#.....................#...#.........#..\n"
            + "                                                  .#..#..............................###.......#..............#....#.......................#....#.....\n"
            + "                                                  .#..........................#..#......#..............#.........#............................#.#.###.\n"
            + "                                                  .................#......#......................................#..........#.....#......##.......#...\n"
            + "                                                  .....##..............#......#.........................#..#...#..#.............#.................#...\n"
            + "                                                  #..#......#..##.....#........#.............#......#...#...............................#....#.......#\n"
            + "                                                  .#....##...........#...#.......#.......#..............#......#........#....#.......#..........#.....\n"
            + "                                                  .....#.........#............#.....##..#.............#.....#...#........#............................\n"
            + "                                                  ..###.......#......#......#.....#....#..#..............................................#............\n"
            + "                                                  ..........#.....................#........................#..#....#............#...#....#............\n"
            + "                                                  ......#..##...............##...............#.............#........#..............#.....#............\n"
            + "                                                  ......#.....................#...........#.......#.#.....................#..............#....#.......\n"
            + "                                                  ........#......#.#.........#...#...........#..#...........#.........................................\n"
            + "                                                  .....#.....#..........#......................#.............#.......#.......#.............#..#.......\n"
            + "                                                  ...#.........#.........#....#...............#.........#...........................#...#.##..#....#.#\n"
            + "                                                  .....#.........##..#...#.###................................#.......##......#.......................\n"
            + "                                                  #........#.........#..#...#...........##...................#.......#....#...........................\n"
            + "                                                  ...#.#......#.............#.#........#.............#...........#..........#......#..#...#......#....\n"
            + "                                                  ...##........#..#.#...............#.....#............##......#.......#..#..#....#...................\n"
            + "                                                  ..........#............#..#.......#..........#.#.................................#..................\n"
            + "                                                  ....#....#............................#..........#.#...............#................................\n"
            + "                                                  ............#.#.#................#.......#...#...........................#..........................\n"
            + "                                                  ....#........#....#..........#......#....#................#..............#........#....#...........#\n"
            + "                                                  .....#.......#........................#.............#...#..##........#..................#...........\n"
            + "                                                  ......##....#..................................................#.#.........#....#......#............\n"
            + "                                                  ..........#.#..................................#.................................#.....#....#.......\n"
            + "                                                  ................#....#.#.#...........#.......#..###..#..................#...#....#....#...#....#....\n"
            + "                                                  .........#....#.#...#.......#..........................................#.........#.#.........#...#..\n"
            + "                                                  .........................#.....#.......................#........#............#.......#...#..........\n"
            + "                                                  .......#.......#.....#.#........................#.\n"
            + "                                                  .................#......#.........#.#.............\n"
            + "                                                  ....##.............#....#.......#..#..........#...\n"
            + "                                                  .............###...................#.#..........#.\n"
            + "                                                  .#.............#...........#......#.....#........#\n"
            + "                                                  .......#.......................#..............#...\n"
            + "                                                  .......#...#......................................\n"
            + "                                                  .......#.#.....#......#...........#..............#\n"
            + "                                                  ...............#......#.#................#........\n"
            + "                                                  .................#.....#.......#.........#..#..#..\n"
            + "                                                  .........#...........................#............\n"
            + "                                                  ..#...#........#.........#.............#.......#..\n"
            + "                                                  .........................#....................#...\n"
            + "                                                  ......#.#..........#........#.....#...............\n"
            + "                                                  ..#.....#..#.....#.................#....#..#......\n"
            + "                                                  ...#.........#........................#.......##..\n"
            + "                                                  ..#.....#..#......................................\n"
            + "                                                  ........#................#.............##.........\n"
            + "                                                  ......#..........#......#.....#..........#........\n"
            + "                                                  ...........#........#...#.........................\n"
            + "                                                  ...................#..........#......#............\n"
            + "                                                  ......#......#.#.#.........................#...#..\n"
            + "                                                  ............##...............#..#.#.#........#....\n"
            + "                                                  ....#...#..#..#...#.....#.........................\n"
            + "                                                  ..........#........#............................#.\n"
            + "                                                  .......#................#.........................\n"
            + "                                                  ...........#.................##.............#.....\n"
            + "                                                  ......#..#.........#......................#.......\n"
            + "                                                  .......#.....#........#...#..#......#...#.......#.\n"
            + "                                                  .........#..#..........#....#...........#.....#...\n"
            + "                                                  ...#..#..............#................#...........\n"
            + "                                                  ......#....................#..#...#.......#.......\n"
            + "                                                  .#..#........#.......#..................#.........\n"
            + "                                                  ............#.....#.#..#.#.#...#.........#...#....\n"
            + "                                                  ...............#.#.#...........#.#............#...\n"
            + "                                                  ..........##......#.........#..............#......\n"
            + "                                                  ....##......#..................#.#................\n"
            + "                                                  ..............#...#................#..............\n"
            + "                                                  .#......#........#.........#.........##...........\n"
            + "                                                  .....................#..................#.#.......\n"
            + "                                                  ........#..#..........#.....................#.....\n"
            + "                                                  ............#..#....#...................#.........\n"
            + "                                                  .......#......#.........#..........#..#.........#.\n"
            + "                                                  ...#.#..........#...............#...##..#.......#.\n"
            + "                                                  ..#.#.#.#...#.##......#.......#..#.#..#...........\n"
            + "                                                  .......#.#.....#......#..#.......#................\n"
            + "                                                  ..#.............................#.#...#....#......\n"
            + "                                                  ..#................#.....#....#.................#.\n"
            + "                                                  ................#.....#.....#......#.............#\n"
            + "                                                  ..............................#............#......\n"
            + "..#...........#.##.#..##..................#....#...##.....................#.....#...................\n"
            + ".....#......#.#...#.....#................#....................#...............................#.....\n"
            + "...#.......................#.......#..#............#.#....##..#...............#..#.#....#.........#.\n"
            + "#...............#.....#..#......##.#.....#......................#.....#...#.........................\n"
            + "...........#.........#....................................................#.............#...........\n"
            + "............##..........#..#...................#..............#...................#....##...........\n"
            + "....#.............##......#..............####.........#....#.......#.................#......#.......\n"
            + "..........#.......#...#.#............................#..................#...........................\n"
            + "........................#..............#............#......#....#...........#......#..........#.....\n"
            + ".#.............................................#..#.#.....................#...............#.........\n"
            + "#.................................................#................#...........#.......#...#........\n"
            + "#.............#.#............##..#...#...........#.............#.................................#..\n"
            + "..................#.......#.....#....#.................#..........#....#.#..#..........#...#........\n"
            + "..........................##..................#...#....................#............................\n"
            + "................#.......#......#.....#.##...........#..#..........#.......#........#..#.............\n"
            + "........#....##....................#................................#.#.....#..#....................\n"
            + ".#..............#..#................#.....#............#....#...............#.......................\n"
            + ".........#.....#................................#.......#....................#..#...................\n"
            + "................#.....#.........#............#.............#..................#......#.....#.##.....\n"
            + "#.........#......................................#..........#........#..#...#...........#...#.......\n"
            + "#.......#..#...#.#.#.#............#..................#...............#..............#.......#.......\n"
            + "........#.....#.....#.....#..#.#...................#....#.......................#...#.....#..#...#..\n"
            + ".........#............................#...........#...#...#.#..........##....#...................#..\n"
            + "......#..................#.................#............#......#..#.................#............##.\n"
            + ".........#...............#..........#..............#..#....#........................#...........#...\n"
            + "..........#....#..#......#..#.............#...#............##............................#.......#..\n"
            + "....#.............#.#..#...........#.#..#..##.#....................................##...............\n"
            + ".....................#.#...##..........#....................#...#................#...#.........#....\n"
            + "...#.#..........................#....#.................#....#.....................#.#...............\n"
            + ".#..........#.........#...............#........##.#................#.......#..............##.#......\n"
            + ".....#........#...#..............#........#...............#....#......................#.............\n"
            + "..........#....#.........................#............#........#...................#............#...\n"
            + "............................................#.............................#...#.....................\n"
            + ".........#........#............#...................#.............#............#...........#.........\n"
            + "..............#.#.#..#............##..................#............#................................\n"
            + "....................#..#.......#............#.................#......#......#........#.#...........#\n"
            + "#....#...##...#.........................##..............#..........#..............#.................\n"
            + ".....#...............#..........#..........#.........#............#..............#..................\n"
            + "............................#...#...........#..#.#.....#.#......#..............#..#.........#.......\n"
            + "......#...........#................#.....................#...#..............#................#....#.\n"
            + ".##..#....#...##........#..................#.#.........#...#.......#.#..........#........#......#...\n"
            + "#.........#.....................##.#...........#....#.#.............#....#........#..#..............\n"
            + ".............................#....#...........#.#....#.#..............#.............#............#.#\n"
            + "........#....#.........#.............#.##.......#...............#.#....#..#.................#......#\n"
            + "...........#..#.......#.#.....#.#.................#..........#..........................#..#..#...#.\n"
            + "............#.....#......#.#.....#...#..##...............#...#......#........................##.....\n"
            + "....##...#.#..........#.........#....#....#.......#...............#....#.........................#..\n"
            + "............#.....#............#....#.....##.......#.....#.........................#...#............\n"
            + ".......#.....#.....................##...........##.......................#.#....#.........#.........\n"
            + "................#.........................................#.........#.#.....#....#.............#....\n"
            + "#.........................#..........#.......#....\n"
            + ".......................#....#.............#.......\n"
            + "...................#........................#.....\n"
            + "....#.#.........#..##..........................#..\n"
            + "..........#.#......#.........#....#...............\n"
            + "..##...........#........#................#........\n"
            + ".......#....#..#...#.##...#.....................#.\n"
            + "......##...#.....#.................#............#.\n"
            + "......#...#...........#....#.......#..............\n"
            + "...#.........#.....#.......#.....##...#.....#.....\n"
            + "..........#....#............#..#..................\n"
            + "..#.#.......................#........#............\n"
            + ".......##.#.##.............#.....#...........#....\n"
            + "..............#....#........#.....#...............\n"
            + "...........................#......#..............#\n"
            + "....#........#.....#......#............#.#.#......\n"
            + ".......#.....#...#................#.....#.........\n"
            + "..............#.........#..........#......#....#..\n"
            + ".#.#..#.....#.........................#......#.#..\n"
            + "......#...#.......#..#..........................##\n"
            + "......#......................................#.#.#\n"
            + ".......#.....#....#.....#..##......#...#..........\n"
            + ".........#..#...#.........#..............#....#...\n"
            + "..#............#.#.............#........#........#\n"
            + ".............#..........#.##......................\n"
            + "#.....#......#....#..................#............\n"
            + "...#........#....................#................\n"
            + "#....#...........##.#......#....#...........##..#.\n"
            + "##...........#..........#....................#....\n"
            + ".#...............#..............#.........#.....#.\n"
            + ".........#...#..................................#.\n"
            + "....................#.#.....#................#....\n"
            + "....#......................................#......\n"
            + "#.................##...#..#...#............#.#.#..\n"
            + "....#........#.#...........#...............#......\n"
            + ".......#...........#....................#.........\n"
            + "...#............##.....#.#............#....#......\n"
            + "#..#...##..#..............#.#.....................\n"
            + "..........#.#......#......##.......#.....#........\n"
            + ".##..................#............................\n"
            + ".....#....................#.......#...............\n"
            + ".................#.#..#...#............#...#.#....\n"
            + "........................#..#.................#....\n"
            + ".#.....#...............##........#.......#........\n"
            + "........#......#.............................#..#.\n"
            + ".........................#...#.......#.......#....\n"
            + "#.......#..................#..#................#..\n"
            + "......#..........#.......##.....#........#........\n"
            + "#.#....#................##......#......#..........\n"
            + "..#.#..............#.........#...................#\n"
            + "\n"
            + "";
}
